function [M_total,S_total,W_total,G_total]= compute_without_self_diff_6F(x,y,h,t_end,dt)
...
mu1 = (0.1/(60*60*24))*scale; % glioma profileration rate due to acidity
...
for j = 1:length(t)
    ...
    for ii = 2:length(x)-1
        for jj = 2:length(y)-1
            ...
            flux_x(ii,jj) =  DivDT_x(ii-1,jj-1)+(alpha*gamma_1)*...
                ((DT_scale-a(ii,jj))*((S_old(ii+1,jj)-S_old(ii-1,jj))/(2*h)) + ...
                (-b(ii,jj))*((S_old(ii,jj+1)-S_old(ii,jj-1))/(2*h)));
            
            flux_y(ii,jj) =  DivDT_y(ii-1,jj-1)+(alpha*gamma_1)*...
    ...
    for kk = 2:Lx-1
        for ll = 2:Lx-1
            ...
            source_M(kk-1,ll-1) = M_old(kk,ll)*(1 - (M_old(kk,ll)/K_M))*((mu1*( 1- (S_old(kk,ll)/K_S))) ...
                + ((mu2*W_old(kk,ll)/K_W))/( 1 + (W_old(kk,ll)/K_W)));
