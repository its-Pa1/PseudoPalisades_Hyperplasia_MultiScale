...
for j = 2:length(t)
    ...
    % glioma diff
    for i = 2:Lx-1
        total_diff_M(i) = DT + ((((DT_scale) - DT)*gamma_2*alpha*M_old(i))*...
    ...
    total_diff_M(1) = DT + (((DT_scale) - DT)*gamma_2*alpha*M_old(1));
    total_diff_M(end) = DT + (((DT_scale) - DT)*gamma_2*alpha*M_old(end));
...
saveas(gcf,'Plots_pattern_dim/Tumor_pattern_dim_6F.png');
%%
figure(2)
surf(x,t,S_total')
view(0,90)
...
axis tight
saveas(gcf,'Plots_pattern_dim/Acidity_pattern_dim_6F.png');
%%
figure(3)
surf(x,t,W_total')
view(0,90)
...
axis tight
saveas(gcf,'Plots_pattern_dim/EC_pattern_dim_6F.png');
%%
% G_total = G_total/(max(max(G_total)));
figure(4)
surf(x,t,G_total')
view(0,90)
colorbar
...
axis tight
saveas(gcf,'Plots_pattern_dim/VEGF_pattern_dim_6F.png');
%%
...
saveas(gcf,'Plots_pattern_dim/Diff_coeff.png');
