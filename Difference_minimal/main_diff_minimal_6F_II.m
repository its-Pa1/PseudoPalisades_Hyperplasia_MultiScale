...
[M_total_old,S_total_old] = compute_minimal_model_set6F(x,y,h,t_end,dt); % glioma and acidity from old model
...
% In this section Videos are saved in Videos_diff_minimal_6F_II folder
% To check if Videos_diff_minimal_6F_II folder exists otherwise to create
if not(isfolder('Videos_diff_minimal_6F_II'))
    mkdir('Videos_diff_minimal_6F_II')
end
...
videofile = VideoWriter(strcat('Videos_diff_minimal_6F_II/tumor_diff_minimal_6F_II', video_ext),v_pro);
...
videofile = VideoWriter(strcat('Videos_diff_minimal_6F_II/acidity_diff_minimal_6F_II', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
videofile = VideoWriter(strcat('Videos_diff_minimal_6F_II/tumor_old_model', video_ext),v_pro);
videofile.Quality = 100;
videofile.FrameRate = 10;
open(videofile);
...
close(videofile);
%% This loop saves the results for each 30 days in folder Plots_diff_minimal_6F_II in png
% or eps format with days in file names

% to check wheather Plots folder exists otherwise it makes a folder Plots
if not(isfolder('Plots_diff_minimal_6F_II'))
    mkdir('Plots_diff_minimal_6F_II')
end
...
for i = 1:10:size(M_diff1,3)
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_Videos_diff_minimal_6F_II/Tumor_diff_minimal_6F_II_%ddays',3*(i-1)),'epsc');%eps
    saveas(gcf,sprintf('Plots_diff_minimal_6F_II/Tumor_diff_minimal_6F_II_%ddays.png',3*(i-1)));%png
    ...
    xlabel('X' , 'Fontsize', 15);
    ylabel('Y' , 'Fontsize', 15);
    %     saveas(gcf,sprintf('Plots_Videos_diff_minimal_6F_II/Acidity_diff_minimal_6F_II_%ddays',3*(i-1)),'epsc');
    saveas(gcf,sprintf('Plots_diff_minimal_6F_II/Acidity_diff_minimal_6F_II_%ddays.png',3*(i-1)))
...
save('Mat_files/main_diff_minimal_6F_II.mat');
%%
